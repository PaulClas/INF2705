...
public:
    Poisson( glm::vec3 pos = glm::vec3(3.0,1.0,0.0), glm::vec3 vit = glm::vec3(1.0,0.0,0.0), float tai = 0.5 )
        : position(pos), vitesse(vit), taille(tai), estSelectionne(false)
    {
    ...
    void afficher()
    {
        matrModel.PushMatrix();{ // sauvegarder la tranformation courante
            ...
            {
                glVertexAttrib4Nub(locColor, 0, 255, 0, 255);
            }
            else
            {
                glVertexAttrib4Nub(locColor, this->couleur[0], this->couleur[1], this->couleur[2], 255);
            }
    ...
    void toggleEstSelectionne() 
    {
        this->estSelectionne = !this->estSelectionne;
    }

    void setCouleur(GLubyte * couleur)
    {
    ...
    GLubyte couleur[3];// glm::vec3 couleur;
    bool estSelectionne;
};
