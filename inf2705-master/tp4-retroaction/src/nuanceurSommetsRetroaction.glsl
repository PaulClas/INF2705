...
void main( void )
{
    if ( tempsDeVieRestant <= 0.0 )
    {
        ...
        tempsDeVieRestantMod = mix( 0, tempsDeVieMax, myrandom(seed++) );
    ...
    else
    {
        ...
        vitesseMod.xy = vitesse.xy;
        vitesseMod.z = vitesse.z - dt * gravite;

        // diminuer son temps de vie
        tempsDeVieRestantMod = max( 0, tempsDeVieRestant - dt );
        ...
        // ...
        vec3 posParticuleSphere = positionMod / bDim;
        float dist = length( posParticuleSphere );
        if (dist >= 1.0)
        {
            positionMod = (2.0 - dist) * positionMod;
            vec3 normale = posParticuleSphere / dist;
            vitesseMod = reflect( vitesseMod, normale );
        }
