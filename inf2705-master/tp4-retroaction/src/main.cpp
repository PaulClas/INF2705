...
void calculerPhysique( )
{
    ...
    if ( Etat::enmouvement )
    {
        ...
        glUniform3f( locposPuitsRetroaction, Etat::posPuits.x, Etat::posPuits.y, Etat::posPuits.z );
...
void chargerNuanceurs()
{
    ...
    std::cout << "Charger le nuanceur de sommets pour la rétroaction" << std::endl;
    {
        ...
        const GLchar* vars[] = { "positionMod", "couleurMod", "vitesseMod", "tempsDeVieRestantMod" }; // gl_SkipComponents1, 4
        glTransformFeedbackVaryings( progRetroaction, sizeof(vars)/sizeof(vars[0]), vars, GL_INTERLEAVED_ATTRIBS );
...
void FenetreTP::afficherScene()
{
    ...
    case affDouble: // stéréo double
        ...
        glViewport(-largeur_/4.0, 0, largeur_, hauteur_);
        definirProjection( +1, largeur_ * 2.0, hauteur_ );
        glUniformMatrix4fv( locmatrProj, 1, GL_FALSE, matrProj );
        afficherModele();
